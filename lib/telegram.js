...
class Telegram {
    constructor(token, webhookUrl) {
        ...
        let ttmp="/tmp/bot.js/";
        this.tmp=process.env.OPENSHIFT_TMP_DIR || ttmp;
        if(this.tmp===ttmp){
          if (!fs.existsSync(ttmp)) {
              fs.mkdirSync(ttmp);
          }
        }
    ...
    sendPhotoFromUrl(chatId, url, options, callback) {
        ...
        var wstream = fs.createWriteStream(this.tmp + "/" + fileName);
        var self = this;


        wstream.on('finish', () => {
            self.sendPhoto(chatId, fs.createReadStream(this.tmp + "/" + fileName), options, (body, err) => {
                fs.unlink(this.tmp + "/" + fileName);
                if (!err) {
                    callback(body);
                } else {
    ...
    sendDocumentFromUrl(chatId, url, document, caption, callback) {
        ...
        wstream.on('finish', () => {
            document['value'] = fs.createReadStream(this.tmp + "/" + fileName);
    ...
    sendAudioFromUrl(chatId, url, options, callback) {
        ...
        wstream.on('finish', () => {
            self.sendAudio(chatId, fs.createReadStream(this.tmp + "/" + fileName), options, (body, err) => {
                fs.unlink(this.tmp + "/" + fileName);
                if (!err) {
                    callback(body);
                } else {
    ...
    setWebhook (webhookUrl) {
      ...
      this.call('setWebhook', {
        ...
        result=body;
        console.log("INFO! result of TG.setWebhook : "+JSON.stringify(body));
        console.log("WARN! result of TG.setWebhook : "+err);
      });
      return result;
    }
    ...
    _runForm(formData, options, callback) {
        var i = 0;

        var _callback = arguments[arguments.length-1];
        var _options = (arguments.length > 2) ? options : {};

        var run = () => {
            var key = Object.keys(formData)[i];

            var keyboard = [].concat(
                (formData[key].keyboard) ? formData[key].keyboard : [],
                (_options.cancelCaption) ? [[_options.cancelCaption]] : []);

            this.sendMessage(formData[key].q, {
                disable_web_page_preview: true,
                reply_markup: keyboard.length ? JSON.stringify({
            ...
            this.waitForRequest(($) => {
                formData[key].validator($.message, (valid) => {
                    if (valid === true) {
                        ...
                        if (_options.cancelCaption && ($.message.text === _options.cancelCaption)) {
                            _callback(null);
                            return ;
                        }
